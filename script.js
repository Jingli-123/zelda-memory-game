...
//get random interger number.
function getRandomIndex(length) {
  return Math.floor(Math.random() * length);
}
//get random element from possibleCardFaces array
...
function hidScoreBoard(scoreBoard) {
...
function reset(
  ...
  score = 0;
  lowScoreOutput.textContent = `Low Score: ${lowScore}`;
  gameClicks.textContent = `Total Click: ${score}`;
...
document.addEventListener("DOMContentLoaded", function () {
  //get element from HTML and store in a variable or an array
  ...
  let highScoreOutput = document.querySelector(".high-score");
  let winScreen = document.getElementById("win-screen");
  let replay = document.getElementById("replay-button");

  assignLowScore(lowScoreOutput);
  //shuffle cards to get random card face.
  shuffleCards(cardFaces);
